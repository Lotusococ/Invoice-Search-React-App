...
// Components Imports
import { SearchBox } from "src/components/SearchBox/Single";
...
import { requestInvoiceSearchWebAPI } from "src/interface/Invoice Search/InvoiceWebAPI/requestInvoiceSearchWebAPI";
...
export const SingleSearch: React.FC = () => {
    ...
    const handleSearch = async (searchValues: string[]) => {
        try {
            ...
            const searchApi = localStorage.getItem('searchApi');

            let requestInvoiceResult = searchApi === 'excelAPI'
                ? await requestInvoiceSearchExcelAPI(validSearchValues)
                : searchApi === 'webAPI'
                    ? await requestInvoiceSearchWebAPI(validSearchValues)
                    : console.error('Invalid searchApi value');
            ...
            sendCustomTraceLog(user, "Failed while handleSearch: " + error, "error", "SingleSearch/index.tsx");
            setLoading(false);
        };
    };
